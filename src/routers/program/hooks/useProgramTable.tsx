...
import { getPrograms, deleteProgram } from '../service';
...
export const useProgramTableConfig = ({ setProgramCount, setCurrentPageCount, filterKeys = {} }: UseProgramTableConfigProps) => {
  ...
  const deleteEntityMutation = useMutation({
    mutationFn: deleteProgram || (() => Promise.reject(new Error('Delete function not provided'))),
  });
  ...
  const { data: entityResponse, isFetching: isLoading } = useQuery({
    ...
    queryFn: () => getPrograms(queryParams as any),
  ...
  if (!primaryKeys || !primaryKeys[programConstants.PRIMARY_KEY]) {
    console.error('Cannot delete: Missing primary keys');
    return;
  }
  await deleteEntityMutation.mutateAsync(primaryKeys);
  queryClient.invalidateQueries({ queryKey: [programConstants.QUERY_KEY, queryParams], exact: false });
  dispatch(resetSelectedObj(programConstants.ENTITY_KEY));
  ...
  const actions: TableAction<IProgramIndex>[] = useMemo(() => {
    ...
    list.push({
      ...
      permission: {
        ...
        module: programConstants.PERMISSIONS.MODULE,
        resource: programConstants.PERMISSIONS.RESOURCE,
        action: programConstants.PERMISSIONS.ACTIONS.EDIT,
      },
    });
    ...
    list.push({
      ...
      permission: {
        ...
        module: programConstants.PERMISSIONS.MODULE,
        resource: programConstants.PERMISSIONS.RESOURCE,
        action: programConstants.PERMISSIONS.ACTIONS.DELETE,
      },
    });
