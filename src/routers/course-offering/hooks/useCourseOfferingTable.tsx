...
import { getCourseOfferings, deleteCourseOffering } from '../service';
...
export const useCourseOfferingTableConfig = ({ setCourseOfferingCount, setCurrentPageCount, filterKeys = {} }: UseCourseOfferingTableConfigProps) => {
  ...
  const deleteEntityMutation = useMutation({
    mutationFn: deleteCourseOffering || (() => Promise.reject(new Error('Delete function not provided'))),
  });
  ...
  const { data: entityResponse, isFetching: isLoading } = useQuery({
    ...
    queryFn: () => getCourseOfferings(queryParams as any),
  ...
  if (!primaryKeys || !primaryKeys[courseOfferingConstants.PRIMARY_KEY]) {
    console.error('Cannot delete: Missing primary keys');
    return;
  }
  await deleteEntityMutation.mutateAsync(primaryKeys);
  queryClient.invalidateQueries({ queryKey: [courseOfferingConstants.QUERY_KEY, queryParams], exact: false });
  dispatch(resetSelectedObj(courseOfferingConstants.ENTITY_KEY));
  ...
  const actions: TableAction<ICourseOfferingIndex>[] = useMemo(() => {
    ...
    list.push({
      ...
      permission: {
        ...
        module: courseOfferingConstants.PERMISSIONS.MODULE,
        resource: courseOfferingConstants.PERMISSIONS.RESOURCE,
        action: courseOfferingConstants.PERMISSIONS.ACTIONS.EDIT,
      },
    });
    ...
    list.push({
      ...
      permission: {
        ...
        module: courseOfferingConstants.PERMISSIONS.MODULE,
        resource: courseOfferingConstants.PERMISSIONS.RESOURCE,
        action: courseOfferingConstants.PERMISSIONS.ACTIONS.DELETE,
      },
    });
