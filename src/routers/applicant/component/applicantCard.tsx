...
interface ApplicantCardProps {
  ...
  actions?: TableAction<IApplicantIndex>[];
  scope: string[];
}

export const ApplicantCard: React.FC<ApplicantCardProps> = ({ record, actions = [], scope }) => {
  const getInitials = () => {
    const init = record.firstName || 'a';
    return init.toString().substring(0, 2).toUpperCase();
  };

  const filteredActions = actions.filter((action: TableAction<IApplicantIndex>) => {
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-gray-900 text-lg truncate">{record.firstName || 'Unknown Applicant'}</h3>
              <p className="text-sm text-gray-500 flex items-center mt-1">
                <Hash className="w-3 h-3 mr-1 flex-shrink-0" />
                <span className="truncate">{record.applicantId || 'N/A'}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        {filteredActions.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <div className="flex gap-2">
              {filteredActions.map((action: TableAction<IApplicantIndex>, index: number) => (
                <Button
