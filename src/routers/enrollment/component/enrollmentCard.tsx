...
interface EnrollmentCardProps {
  ...
  actions?: TableAction<IEnrollmentIndex>[];
  scope: string[];
}

export const EnrollmentCard: React.FC<EnrollmentCardProps> = ({ record, actions = [], scope }) => {
  const getInitials = () => {
    const init = record.studentIdInSi || 'e';
    return init.toString().substring(0, 2).toUpperCase();
  };

  const filteredActions = actions.filter((action: TableAction<IEnrollmentIndex>) => {
      ...
      <CardContent className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-gray-900 text-lg truncate">{record.studentIdInSi || 'Unknown Enrollment'}</h3>
              <p className="text-sm text-gray-500 flex items-center mt-1">
                <Hash className="w-3 h-3 mr-1 flex-shrink-0" />
                <span className="truncate">{record.enrollmentId || 'N/A'}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center text-sm text-gray-700 bg-gradient-to-r from-pink-50 to-purple-50 p-3 rounded-lg">
            ...
            <span className="truncate font-medium">{record.studentIdInSi}</span>
          </div>
        </div>
        ...
        {filteredActions.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <div className="flex gap-2">
              {filteredActions.map((action: TableAction<IEnrollmentIndex>, index: number) => (
                <Button
